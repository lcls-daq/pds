...
void Pds::Epix100aConfig::setDaqCode(Epix100aConfigType& c, uint32_t daqCode)
{
  char* buffer = new char[c._sizeof()];
  memcpy(buffer, &c, c._sizeof());
  Epix100aConfigType* tmp = reinterpret_cast<Epix100aConfigType*>(buffer);
  new(&c) Epix100aConfigType(tmp->version(),
                             tmp->usePgpEvr(),
                             tmp->evrRunCode(),
                             daqCode,
                             tmp->evrRunTrigDelay(),
